    ...
    private void onEksportirajfClick(ActionEvent event) {
        if (popisSvihTransakcijaRadioButton.isSelected()) {
            FileChooserUtils.showSaveDialog(root,
                    StringUtils.generateFileNameWithCurrentDateTime("Transakcije", LocalDateTimePattern.HR),
                    f -> () -> GenericBeanReportPdf.create("TRANSAKCIJE", transakcijaService.findAll(), propertiesInfo, f.getAbsolutePath(), LanguageFormat.HR),
                    "*.pdf",
                    f -> String.format("Popis transakcija je uspješno eksportiran u PDF. Putanja: %s.", f.getAbsolutePath()),
                    "Desila se greška prilikom eksportiranja popisa transkacija u PDF.");
        }
        ...
        if (izvjestajTransakcijaUZadanomPerioduRadioButton.isSelected()) {
            ...
            if (datumOd != null && datumDo != null) {
                FileChooserUtils.showSaveDialog(root,
                        String.format("Transakcije_U_Periodu_Od_%s_Do_%s", LocalDateTimePattern.HR, datumOd.toString(), datumDo.toString()),
                        f -> () -> transakcijeZadaniPeriodPdfReport.create(datumOd, datumDo, f.getAbsolutePath()),
                        "*.pdf",
                        f -> String.format("Popis transkacija u zadanom periodu je uspješno eksportiran u PDF. Putanja: %s.", f.getAbsoluteFile()),
                        "Desila se greška prilikom eksportiranja popisa transakcija u zadanom periodu u PDF.");
            } else {
