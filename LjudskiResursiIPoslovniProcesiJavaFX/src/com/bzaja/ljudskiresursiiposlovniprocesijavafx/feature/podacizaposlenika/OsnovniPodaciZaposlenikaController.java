...
public class OsnovniPodaciZaposlenikaController implements Initializable, ControllerInterface<PrijavljeniZaposlenikDto> {
    ...
    private void onPromjeniLozinkuClick(ActionEvent event) {
        PromjenaLozinkeZaposlenikaController promjenaLozinkeZaposlenikaController = stageManager.showSecondaryStage(FxmlView.PROMJENA_LOZINKE_ZAPOSLENIKA, prijavljeniZaposlenikDto.getZaposlenik().getIdZaposlenik(), AppUtils.getCssPathables());
    }
    ...
    private void onUrediClick(ActionEvent event) {
        ...
        if (urediZaposlenikaController.getStageResult().equals(StageResult.OK)) {
            DialogUtils.tryShowErrorDialog(() -> refreshZaposlenikData(zaposlenikService.findById(editZaposlenikDto.getIdZaposlenik())), "Desila se greška prilikom dohvaćanja zaposlenika.");
        }
    }
